	...
	if( !$mysqli->connect_errno )
	{
		if( $result = $mysqli->query( "select distinct courseCode from Courses order by courseCode;"))
		{
			...
			if(!empty( $_POST ) && !empty( $_POST[ "courseSelect" ]))
			{
				...
				{
					echo "<h1>All sessions of CS $_POST[courseSelect] in the last " . (($selectedYear > 1) ? ($selectedYear . " years") : "year") . "</h1>";
					$targetYear = $year - $selectedYear;
					if( $result2 = $mysqli->query( "select catalogYear, sectionNumber, firstName, lastName, enrollment, semester, year from consistsOf natural join Courses natural join Sections natural join taughtBy natural join Instructors where courseCode=$_POST[courseSelect] and year > $targetYear order by catalogYear desc, semester desc;" ))
					{
